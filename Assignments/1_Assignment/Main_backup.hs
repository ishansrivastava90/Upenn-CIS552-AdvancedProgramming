...
module Main where
import Prelude  hiding (reverse, takeWhile, zip, concat, concatMap)
import Test.HUnit 
...
takeWhile' :: (a -> Bool) -> [a] -> [a]
takeWhile' p = snd . foldl prefix (True, [])
      where prefix (continue, accu) elem
          | continue && p elem = (continue, accu ++ [elem])
          | otherwise = (False, accu)
