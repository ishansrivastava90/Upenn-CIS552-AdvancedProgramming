...
Exercise: We have just built monad transformers for both exception-handling
and state. We can thus build the monad `StateIntT Exc` and `ExcT Prof`. But
both of those examples require a non-monad-transformer at the root. It is
a pain to build both monad transformers and the regular monad, especially
when the monad transformer seems more general.

The way forward here is to write an Identity monad, like this:

> newtype Identity a = Id { unId :: a }

Note that this datatype does very little -- it just wraps its payload on the
`Id` constructor. But we can write a `Monad` instance (& `Functor` & `Applicative`)
for it. Write that instance here:
...
> goExSt :: Expr -> IO ()
> goExSt e = putStr $ prExSt (evalExSt e)
>
> goStEx :: Expr -> IO ()
> goStEx e = putStr $ prStEx (evalStEx e)

>-- newtype StateIntT m a =  MkStateIntT { runStateIntT :: Int -> m (a, Int) }
>
> prExSt :: StateIntT Exc Int -> String
> prExSt MktateInt a = show (runStateIntT a 0)
>
> prStEx :: ExcT Prof Int -> String
> prStEx = show (runProf (runExcT x) 0) 
