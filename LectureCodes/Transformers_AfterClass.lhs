...
> data Mega a = Mega { runMega :: Int -> Exc (a, Int) }
>
> instance Monad Mega where
>   return x = Mega $ \s -> Result (x, s)
>   ma >>= fmb = Mega $ \s -> case runMega ma s of
>                               Raise blah -> Raise blah
>                               Result (a, s') -> runMega (fmb a) s'
>
>
...
>   fmap = liftM
>
> instance MonadErrorString Mega where
>   throwErrorString str = Mega $ \_ -> Raise str
> instance MonadStateInt Mega where
>   -- getInt :: Mega Int
>   getInt = Mega $ \s -> Result (s, s)
>   putInt x = Mega $ \_ -> Result ((), x)
>   runStateInt m s = case runMega m s of
>                       Raise str -> throwErrorString str
>                       Result (a, s')  -> return (a, s')
>
...
>   -- p :: m (Exc a)
>   MkExc p >>= f  = MkExc $ p >>= (\a -> case a of
>                                           Raise str -> return (Raise str)
>                                           Result a' -> runExcT (f a') )
>{-
>
>
>     MkExc $ do ea <- p    -- ea :: Exc a
>                                case ea of
